...
from utils.helpers.request_executor import shared_dict, add_to_shared_dict
...
class ThreeDsPage(BasePage):
    ...
    @staticmethod
    def get_challenge_display_mode_val_from_config():
        return shared_dict[SharedDictKey.RAW_E2E_CONFIG.value]['threeDSecure']['challengeDisplayMode']
    ...
    def cancel_3ds_challenge(self):
        if 'POPUP' in self.get_challenge_display_mode_val_from_config():
            self._actions.click(ThreeDSMethodsLocators.three_ds_challenge_popup_cancel_button)
        else:
            self._actions.click(ThreeDSMethodsLocators.three_ds_challenge_inline_cancel_button)

    def get_3ds_challenge_cancel_btn_text(self):
        if 'POPUP' in self.get_challenge_display_mode_val_from_config():
    ...
    def validate_3ds_challenge_cancel_btn_translation(self, expected_translation):
        actual_translation = self.get_3ds_challenge_cancel_btn_text()
    ...
    def validate_3ds_challenge_cancel_btn_translation_locale(self, locale_code):
        expected_translation = get_translation_from_json(locale_code, 'cancel')
        self.validate_3ds_challenge_cancel_btn_translation(expected_translation)
