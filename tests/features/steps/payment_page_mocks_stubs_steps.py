...
@step('{request_type} Visa Checkout mock response is set to SUCCESS')
def step_impl(context, request_type):
...
@step('ACS mock response is set to "{acs_response}"')
...
@step('User clicks Pay button - {request_type} response is set to "{action_code}"')
...
@step('{request_type} response is set to "{action_code}"')
def step_impl(context, request_type, action_code):
    stub_st_request_type(AUTHresponse[action_code].value, RequestType.AUTH.name)


@step('Frictionless THREEDQUERY, AUTH response is set to {action_code}')
def step_impl(context, action_code):
    stub_st_request_type(frictionless_request_type[action_code], 'THREEDQUERY, AUTH')


@step('{request_type} mock response for tokenisation is set to OK')
def step_impl(context, request_type):
...
@step('{request_type} ware sent only once in one request')
def step_impl(context, request_type):
...
@step('{request_type} request was not sent')
def step_impl(context, request_type):
    validate_number_of_requests_without_data(context, request_type, 0)


@then('{request_type} request was sent {multiple} time')
...
@step('{thirdparty} or AUTH requests were sent only once with correct data')
...
@step('{request_type} request was sent only once {scenario} \'fraudcontroltransactionid\' flag')
...
@step('{request_type} request for {thirdparty} is sent only once with correct data')
...
@step('{request_type} requests contains updated jwt')
def step_impl(context, request_type):
    payment_page = context.page_factory.get_page(Pages.PAYMENT_METHODS_PAGE)
