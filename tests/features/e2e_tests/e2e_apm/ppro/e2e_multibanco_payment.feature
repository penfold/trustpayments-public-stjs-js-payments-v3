...
@MULTIBANCO
@STJS-2594
Feature: E2E MULTIBANCO Payments
  As a user
  I want to use MULTIBANCO payment
...
#      | RISKDEC ACCOUNTCHECK JSINPT AUTH SUBSCRIPTION       |
#      | JSINPT AUTH                                         |
...
#      | RISKDEC ACCOUNTCHECK JSINPT AUTH                    |
...
#      | JSINPT AUTH FRAUDSCREENING                          |
#      | FRAUDSCORE JSINPT AUTH                              |
#      | ACCOUNTCHECK JSINPT AUTH                            |
#      | RISKDEC AUTH SUBSCRIPTION                           |
#      | ACCOUNTCHECK JSINPT AUTH SUBSCRIPTION               |
#      | RISKDEC2 ACCOUNTCHECK AUTH                          |
#      | THREEDQUERY AUTH FRAUDSCREENING                     |
#      | ORDERDETAILS AUTH                                   |
#      | JSINPT AUTH SUBSCRIPTION                            |
#      | AUTH SUBSCRIPTION                                   |
#      | RISKDEC ACCOUNTCHECK THREEDQUERY AUTH               |
#      | RISKDEC JSINPT AUTH                                 |
#      | RISKDEC2 ACCOUNTCHECK THREEDQUERY AUTH SUBSCRIPTION |
#      | RISKDEC JSINPT AUTH SUBSCRIPTION                    |
...
#      | THREEDQUERY AUTH RISKDEC2                           |
  ...
  Scenario: successRedirectUrl and parameters verification
    ...
    And User opens example page WITH_APM
    And User focuses on APM payment methods section
    And User chooses MULTIBANCO from APM list
...
#      | orderreference         | 123456 |  commented on purpose
  ...
  Scenario: default configuration override
    Given JS library configured by inline config BASIC_CONFIG
    And JS library configured by inline configAPMs MULTIBANCO_CONFIG_APM
    And JS library authenticated by jwt BASE_JWT with additional attributes
      | key                     | value     |
      | requesttypedescriptions | AUTH      |
      | baseamount              | 70        |
    ...
    And User opens example page WITH_APM
    And User focuses on APM payment methods section
    And User chooses MULTIBANCO from APM list - override placement
    And User will be sent to apm page - simulator
    When User will select Failed Unknown response and submit
