...
Feature: 3ds SDK v2 E2E tests with redirection after payment - AMEX v2.1
  ...
  Scenario Outline: TC_6b - Authentication success after retry when DS timeout in first call - Card: AMEX_V21 Request types: <request_types>
  ...
  Scenario Outline: TC_14 - successful frictionless with transaction timed out error for method url- Card: AMEX_V21 Request types: <request_types>
    Given JS library configured with BASIC_CONFIG and additional attributes
      | key             | value |
      | submitOnSuccess | true  |
    Given JS library authenticated by jwt BASE_JWT with additional attributes
    ...
    And User opens example page
    And User waits for form inputs to be loaded
    When User fills payment form with defined card AMEX_V21_3DS_SDK_TRANSACTION_TIMEOUT
    And User clicks Pay button
    Then User will be sent to page with url "www.example.com" having params
      | key                  | value                                   |
      | errormessage         | Payment has been successfully processed |
  ...
  Scenario Outline: TC_4a - successful frictionless with transaction timed out at athe ACS - Card: AMEX_V21 Request types: <request_types>
    Given JS library configured with BASIC_CONFIG and additional attributes
      | key          | value |
      | <e2e_config> | true  |
    Given JS library authenticated by jwt BASE_JWT with additional attributes
    ...
    And User opens example page
    And User waits for form inputs to be loaded
    When User fills payment form with defined card AMEX_V21_3DS_SDK_TRANS_STATUS_AUTH_FAILED
    And User clicks Pay button
  ...
  Scenario Outline: TC_4d - successful frictionless with transaction timed out at the ACS - Card: AMEX_V21 Request types: <request_types>
    Given JS library configured with BASIC_CONFIG and additional attributes
      | key             | value |
      | submitOnSuccess | true  |
    Given JS library authenticated by jwt BASE_JWT with additional attributes
    ...
    And User opens example page
    And User waits for form inputs to be loaded
    When User fills payment form with defined card AMEX_V21_3DS_SDK_TRANS_STATUS_TRANSACTION_TIMEOUT_AT_ACS
    And User clicks Pay button
    Then User will be sent to page with url "www.example.com" having params
      | key                  | value                                   |
      | errormessage         | Payment has been successfully processed |
