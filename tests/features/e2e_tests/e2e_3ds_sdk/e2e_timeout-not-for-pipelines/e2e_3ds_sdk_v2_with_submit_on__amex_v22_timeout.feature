...
Feature: 3ds SDK v2 E2E tests with redirection after payment - AMEX v2.2
  ...
  Scenario Outline: TC_4a - successful frictionless with transaction timed out at athe ACS - Card: AMEX_V22 Request types: <request_types>
    Given JS library configured with BASIC_CONFIG and additional attributes
      | key          | value |
      | <e2e_config> | true  |
    Given JS library authenticated by jwt BASE_JWT with additional attributes
  ...
  Scenario Outline: TC_4d - successful frictionless with transaction timed out at the ACS - Card: AMEX_V22 Request types: <request_types>
    Given JS library configured with BASIC_CONFIG and additional attributes
      | key             | value |
      | submitOnSuccess | true  |
    Given JS library authenticated by jwt BASE_JWT with additional attributes
