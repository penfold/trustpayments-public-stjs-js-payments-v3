...
Feature: 3ds SDK v2 E2E tests - MasterCard v2.2
  ...
  Scenario Outline: TC_6b - Authentication success after retry when DS timeout in first call - Card: MASTERCARD_V22 Request types: <request_types>
    Given JS library authenticated by jwt BASE_JWT with additional attributes
      | key                     | value              |
      | requesttypedescriptions | <request_types>    |
      | sitereference           | jstrustthreed76424 |
  ...
  Scenario Outline: TC_4a - transaction timed out at athe ACS - Card: MASTERCARD_V22 Request types: <request_types>
    Given JS library authenticated by jwt BASE_JWT with additional attributes
      | key                     | value              |
      | requesttypedescriptions | <request_types>    |
      | sitereference           | jstrustthreed76424 |
...
#      | ACCOUNTCHECK THREEDQUERY | Payment has been successfully processed | success  | disabled | Timout in acs/gateway takes more time than wait in tests
  ...
  Scenario Outline: TC_4d - transaction timed out at the ACS - Card: MASTERCARD_V22 Request types: <request_types>
    Given JS library authenticated by jwt BASE_JWT with additional attributes
      | key                     | value              |
      | requesttypedescriptions | <request_types>    |
      | sitereference           | jstrustthreed76424 |
