...
describe('GooglePaymentMethod', () => {
  ...
  beforeEach(() => {
    ...
    when(threeDProcessMock.performThreeDQuery(anything(), anything(), anything(), anything())).thenReturn(of(threeDQueryResponse));
  ...
  describe('start()', () => {
    describe('frictionless payments - with single request to gateway', () => {
      ...
      it('returns payment result with error status if errorcode != 0', done => {
        ...
        when(threeDProcessMock.performThreeDQuery(anything(), anything(), anything(), anything())).thenReturn(of(errorResponse));
