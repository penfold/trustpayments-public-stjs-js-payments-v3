...
describe('TokenizedCardPaymentMethod', () => {
  ...
  describe('on start()', () => {
    ...
    it('should return payment result with error status if processing service throws an error', done => {
      const someError = new Error();

      when(processingServiceMock.process(anything())).thenThrow(someError);

      sut.start(startData).subscribe({
        error: (error) => {
    ...
    it('should publish BLOCK_FORM event on error to unblock the form', done => {
      const someError = new Error();
