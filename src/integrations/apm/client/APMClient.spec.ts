    ...
    it('should generate APM buttons from provided config object and insert each button to its respective container', done => {
      apmClient.init(testConfig).subscribe(() => {
        expect(document.body.innerHTML).toEqual(
          `<div id="test-placement"><div class="st-apm-button"><img src="${zipIconEncoded}" alt="${APMName.ZIP}" id="ST-APM-${APMName.ZIP}" class="st-apm-button__img"></div></div><div id="test-placement-2"><div class="st-apm-button"><img src="${zipIconEncoded}" alt="${APMName.ALIPAY}" id="ST-APM-${APMName.ALIPAY}" class="st-apm-button__img"></div></div><div id="test-placement-3"><div class="st-apm-button st-apm-button--withButton"><button class="st-apm-button__button" id="ST-APM-${APMName.ACCOUNT2ACCOUNT}" style="min-width: ${APMA2AButtonConfig.width}; height: ${APMA2AButtonConfig.height}; background-color: ${APMA2AButtonConfig.backgroundColor}; color: ${APMA2AButtonConfig.textColor};" type="button"><span>${APMA2AButtonConfig.text}</span></button></div></div>`
        );
        done();
      });
    });
